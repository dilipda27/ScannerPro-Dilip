...
with tab_option_desk:
    if st.session_state.get("main_tabs") == "📈 Option Desk":
    ...
    with tab_analytics:
        if st.session_state.get("main_tabs") == "📊 Performance Analytics":
            import analytics_helper
            analytics_helper.render_analytics_tab()

    with tab_intraday:
        if st.session_state.get("main_tabs") == "💼 Intraday Paper Trades":
    ...
    with tab_swing:
        if st.session_state.get("main_tabs") == "📊 Swing Trades":
    ...
    with tab_backtest:
        if st.session_state.get("main_tabs") == "📉 Backtesting":
...
with tab_scanners:
    if st.session_state.get("main_tabs") == "🔍 Scanners":

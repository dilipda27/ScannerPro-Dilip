...
def run_automated_orb(scan_label="ORB Scan"):
    ...
    try:
        ...
        if results_df.empty:
            logging.info(f"✅ {scan_label} complete: No breakout setups detected.")
            return
...
def run_automated_52wh():
    ...
    try:
        ...
        if results_df.empty:
            logging.info("✅ 52-Week High Scan complete: All breakout candidates already have active trades running.")
            return
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            # If entry window is open, run intraday scanners concurrently
            if entry_open:
                scanner_funcs = [
                    (run_automated_52wh, "52WH"),
                    (run_automated_bearish_vwap_rejection, "Bearish VWAP"),
                    (run_automated_bullish_vwap_rejection, "Bullish VWAP"),
                    (run_automated_failed_breakouts, "Failed Breakouts"),
                    (run_automated_bullish_breakouts, "Bullish Breakouts"),
                    (run_automated_bearish_breakdowns, "Bearish Breakdowns"),
                    (run_automated_morning_range, "Morning Range"),
                    (lambda: run_automated_orb("Continuous ORB"), "ORB"),
                    (run_automated_gainers_losers, "Gainers/Losers")
                ]
                
                import concurrent.futures
                with concurrent.futures.ThreadPoolExecutor(max_workers=len(scanner_funcs)) as executor:
                    futures = {executor.submit(func): name for func, name in scanner_funcs}
                    for future in concurrent.futures.as_completed(futures):
                        try:
                            future.result()
                        except Exception as e:
                            logging.error(f"Continuous loop - {futures[future]} error: {e}")

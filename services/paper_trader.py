...
def add_notification_to_shared(ticker, msg, category="Breakout"):
    """Appends a UI notification alert to the shared JSON file so it appears in Streamlit's Activity Feed."""
    import time
        ...
        try:
            ...
            if os.path.exists(SHARED_NOTIFICATIONS_FILE):
                try:
                    ...
                    data = []
            # Prevent exact duplicate notification within the last few alerts
            is_dup = any(n.get('ticker') == ticker and n.get('msg') == msg for n in data[-5:])
            if not is_dup:
                data.append({
                    "ticker": ticker,
                    "msg": msg,
...
def execute_paper_trade(ticker, trade_type, entry_price, sl, qty, token=None, strategy="15-Min ORB", target=None, delta=None):
    with _trader_lock:
        ...
        # Block same-day re-entry (avoid entering a stock if it was already traded and closed today)
        today_date_str = datetime.now().strftime("%Y-%m-%d")
        
        # Check portfolio records (if any closed items remain in portfolio file)
        closed_today_in_portfolio = not df.empty and (
            (df['Ticker'] == ticker) & 
            (df['Status'].astype(str).str.upper() == 'CLOSED') & 
            (df['EntryTime'].astype(str).str.startswith(today_date_str))
        ).any()
        
        if closed_today_in_portfolio:
            logging.info(f"🚫 Blocked same-day re-entry for {ticker} (already closed in portfolio today)")
            return False
        ...
        if os.path.exists(HISTORY_FILE):
            try:
                ...
                if not hist_df.empty and 'Ticker' in hist_df.columns and 'EntryTime' in hist_df.columns:
                    already_traded_today = (
                        (hist_df['Ticker'] == ticker) & 
                        (hist_df['EntryTime'].astype(str).str.startswith(today_date_str))
                    ).any()
                    if already_traded_today:
                        logging.info(f"🚫 Blocked same-day re-entry for {ticker} (already traded and logged in history today)")
                        return False
            except Exception as e:
                logging.warning(f"Error checking history for same-day re-entry: {e}")
        ...
        try:
            msg = f"🟢 {trade_type} Entry @ {entry_price:.2f} (SL: {sl:.2f}, Target: {target:.2f})"
            add_notification_to_shared(ticker, msg, category="Bullish" if "Bullish" in trade_type or "Long" in trade_type else "Bearish")
        except Exception as ne:
            logging.error(f"Failed to add entry notification: {ne}")
        return True
...
def exit_trade(ticker, kite, override_price=None, entry_time=None):
    ...
    with _trader_lock:
        ...
        try:
            ...
            try:
                msg = f"🔴 Exit Executed: {trade['Type']} @ {actual_exit_price:.2f} (PnL: {trade['Final P&L']:.2f})"
                add_notification_to_shared(ticker, msg, category="Exit")
            except Exception as ne:
                logging.error(f"Failed to add exit notification: {ne}")
            return True
        except Exception as e:
...
def archive_history():
    ...
    if os.path.exists(PORTFOLIO_FILE):
        try:
            ...
            _safe_to_csv(pdf, PORTFOLIO_FILE)
        except Exception as e:
...
def apply_multi_stage_trailing_sl(row, ltp, now_time=None):
    """
    Applies multi-stage trailing stop-loss logic with strategy-aware breakeven thresholds
    (1.25R for Breakout strategies, 1.0R for Rejection strategies) and post-2:30 PM (14:30)
    ...
    try:
        ...
        # Breakout strategies get a wider Stage 1 Breakeven threshold (1.25R) to avoid premature retest chop-outs.
        # Rejection / Mean-reversion strategies keep 1.0R breakeven threshold.
        ...
        # Time check for post-2:30 PM (14:30) profit-locking rule
        now = now_time if now_time is not None else datetime.now()
        is_post_1430 = now.hour > 14 or (now.hour == 14 and now.minute >= 30)

        if "Bullish" in trade_type:
            initial_risk = entry - initial_sl
            if initial_risk <= 0: return current_sl
            
            # Post-2:30 PM Profit-Lock Rule: Lock 50% of unrealized gains if gain > 0.5R
            if is_post_1430 and ltp >= entry + (0.5 * initial_risk):
                profit_lock_sl = entry + (0.5 * (ltp - entry))
                if profit_lock_sl > current_sl:
            ...
            # Stage 2: Price >= +1.5R -> Trail to +0.5R
                ...
                if new_sl > current_sl: return new_sl
            # Stage 1: Price >= +be_r_threshold R -> Trail to Break-Even (Entry)
            elif ltp >= entry + (be_r_threshold * initial_risk):
            ...
            # Post-2:30 PM Profit-Lock Rule: Lock 50% of unrealized gains if gain > 0.5R
            if is_post_1430 and ltp <= entry - (0.5 * initial_risk):
                profit_lock_sl = entry - (0.5 * (entry - ltp))
                if profit_lock_sl < current_sl:
                    current_sl = profit_lock_sl

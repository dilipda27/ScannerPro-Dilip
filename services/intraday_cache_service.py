...
class IntradayCacheService:
    def __init__(self):
        ...
        self.task_list = [
            ...
            {"id": "unified_morning", "name": "Unified Morning Cache (ORB, 52WH, Bullish, Bearish, Failed)", "scheduled_time": "09:05", "run_missed": True},
    ...
    def execute_task(self, task_id, kite):
        ...
        def cb(processed, total, symbol):
            ...
            if processed % 15 == 0 or processed == total:
                self.log(f"Progress: {processed}/{total} ({pct:.1f}%) | Current symbol: {symbol}")

        try:
            if task_id == "archive":
                self.log("📋 Phase: Archiving paper trade history of yesterday...")
                ...
                self.log("📋 Phase: Unified Morning Cache. System checking:")
                self.log("  🔹 52-Week High Breakout: Price within 3% of 250-day high, EMA20 > EMA50 > EMA200 uptrend.")
                self.log("  🔹 15-Min ORB Breakout: NR4/NR7/Inside contraction, ATR% >= 2.0%, EMA20 & RSI14 trend alignment.")
                self.log("  🔹 Bullish Strength: Price > EMA50, RSI14 > 50.")
                self.log("  🔹 Bearish Breakdown: Price < EMA50 or RSI14 < 55.")
                self.log("  🔹 Failed Breakout: Price < EMA20, RSI14 <= 48, yesterday's high resistance rejection wick.")
                ...
                self.log("📋 Phase: Full F&O Strength Cache. System checking:")
                self.log("  🔹 Price > daily EMA50.")
                self.log("  🔹 Daily RSI14 > 50.")
                ...
                self.log("📋 Phase: Full Bearish Breakdown Cache. System checking:")
                self.log("  🔹 Price < daily EMA50 or Daily RSI14 < 55.")
            ...
            elif task_id == "failed_breakout":
                self.log("📋 Phase: Full Failed Breakout Cache. System checking:")
                self.log("  🔹 Price < EMA20, RSI14 <= 48.0 (bearish structure).")
                self.log("  🔹 Yesterday's high resistance rejection wick (upper wick >= 25% of candle range).")
                
                from strategies import failed_breakout_scanner
                failed_breakout_scanner.cache_failed_candidates(kite, progress_callback=cb, refresh_only=False)
                self.log("✅ Full Failed Breakout Cache complete.")
                
            elif task_id == "vcp_caching":
                self.log("📋 Phase: VCP Stage 1 & Stage 2 Caching. System checking:")
                self.log("  🔹 Stage 1 EOD Proximity: 20-day High/Low proximity (within 3%), Vol SMA > 500k, tight range (<= 12%), EMA50 & RSI trend.")
                self.log("  🔹 Stage 2 Validation: 5-day ATR < 14-day ATR, 5-day ATR/price <= 3.5%, volume SMA contraction (5D < 20D), price flat closes, contracting pivot waves.")
                ...
                self.log("📋 Phase: Refreshing ORB Cache. System checking:")
                self.log("  🔹 Updating breakouts using today's open price action.")
                from strategies import kite_scanner
                ...
                self.log("📋 Phase: Refreshing Bullish Breakout Cache. System checking:")
                self.log("  🔹 Filtering candidates: today's open/momentum (LTP >= Open, LTP >= 99.5% of yesterday's high).")
                from strategies import bullish_breakout_scanner
                ...
                self.log("📋 Phase: Refreshing Bearish Breakdown Cache. System checking:")
                self.log("  🔹 Filtering candidates: today's open/momentum (LTP < Open or LTP < 99.5% of yesterday's low).")
                from strategies import bearish_breakdown_scanner
                ...
                self.log("📋 Phase: Refreshing Failed Breakout Cache. System checking:")
                self.log("  🔹 Filtering candidates: today's open/momentum (LTP <= Yesterday's High * 1.015, today's high >= Yesterday's High * 0.985).")
                from strategies import failed_breakout_scanner
...
def print_caching_summary():
    """Prints a summary in the log/terminal of the number of stocks cached for each scanner."""
    print("\n" + "="*65)
    ...
    for name, path in cache_files.items():
        ...
        if os.path.exists(path):
            try:
                ...
                pass
        status = f"{count} stocks @ {mtime_str}" if count > 0 else "0 stocks (not cached)"
        logging.info(f"[Cache Summary] {name}: {status}")
    ...
    for name, path in json_caches.items():
        count = 0
        mtime_str = "--"
        if os.path.exists(path):
            try:
    ...
    if os.path.exists(mr_watch_path):
        try:
            ...
            if isinstance(data, dict) and "watchlist" in data:
                ...
                mr_watch_count = len(data)
            mtime = datetime.datetime.fromtimestamp(os.path.getmtime(mr_watch_path))
            mr_mtime_str = mtime.strftime("%H:%M")
        except Exception:
            pass
    mr_status = f"{mr_watch_count} stocks @ {mr_mtime_str}" if mr_watch_count > 0 else "0 stocks (not cached)"
    logging.info(f"[Cache Summary] Morning Range Watchlist: {mr_status}")
    print(f" 🔹 {'Morning Range Watchlist':<45} : {mr_status}")
    print("="*65 + "\n")

...
def cache_bullish_candidates(kite, progress_callback=None, refresh_only=False):
    ...
    if refresh_only and os.path.exists(BULLISH_CACHE_FILE):
        ...
        if cache_df.empty:
            logging.info("Existing strength cache is empty. Nothing to refresh.")
            return False
        ...
        if cache_data:
            pd.DataFrame(cache_data).to_csv(BULLISH_CACHE_FILE, index=False)
            logging.info(f"Cache Refreshed. {len(cache_data)} stocks shortlist ready.")
            return True
        else:
            pd.DataFrame(columns=cache_df.columns).to_csv(BULLISH_CACHE_FILE, index=False)
            logging.info("Cache Refreshed. No stocks matched today's momentum.")
            return True
    else:
        ...
        if cache_data:
            pd.DataFrame(cache_data).to_csv(BULLISH_CACHE_FILE, index=False)
            logging.info(f"Cache Updated (Full Scan). {len(cache_data)} stocks shortlist ready.")
            return True
    ...
    for _, row in processing_list.iterrows():
        ...
        try:
            ...
            if vol_spike and above_vwap and not is_chasing and nifty_bullish and is_consolidating:
                ...
                if datetime.time(9, 30) <= to_date.time() <= datetime.time(14, 0) and is_breakout:

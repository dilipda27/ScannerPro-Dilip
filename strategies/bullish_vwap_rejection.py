...
def run_rejection_scanner(df, pdc, yesterday_high=None, nifty_bullish=False):
    ...
    df_session['ema_20_sloping_up'] = (df_session['ema_20'] > df_session['ema_20'].shift(3)).fillna(True)
    
    df_session['trend_ok'] = (df_session['close'] > df_session['vwap']) & \
                             (df_session['vwap'] > pdc) & \
                             (df_session['vwap_sloping_up']) & \
                             (df_session['ema_20_sloping_up']) & \
    ...
    e_pullback = (df_session['low'] <= df_session['ema_20'] + atr_buffer) & \
                 (df_session['high'] >= df_session['ema_20'] - atr_buffer)
    ...
    df_session['ema_dist_pct'] = ((df_session['close'] - df_session['ema_20']) / df_session['ema_20'] * 100).abs()
    ...
    # Broad market Nifty filter (strictly requires confirmed BULLISH trend today)
    df_session['nifty_ok'] = nifty_bullish
    
    # Trend strength filter (strictly requires ADX >= 20)
    df_session['adx_ok'] = df_session['adx_5m'] >= 20
    
    # Color check: Trigger candle must be GREEN (momentum confirmation)
    df_session['candle_is_green'] = df_session['close'] > df_session['open']
...
def scan_all_tickers_parallel(active_df, progress_callback=None, use_demo=False, kite=None, nifty_bullish=False):
    ...
    def scan_single(row):
        ...
        try:
            ...
            df_analyzed, alerts = run_rejection_scanner(df_raw, pdc, yesterday_high=yesterday_high, nifty_bullish=nifty_bullish)
    ...
    def scan_single_with_ctx(row):
        if ctx:
            add_script_run_ctx(ctx=ctx)
        return scan_single(row)

    with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
        futures = {executor.submit(scan_single_with_ctx, row): row['Ticker'] for _, row in active_df.iterrows()}
...
def scan_bullish_vwap_rejections(kite, use_demo=False):
    ...
    nifty_bullish = is_nifty_bullish(kite) if (not use_demo and kite) else False
    ...
    triggered_df, monitored_df = scan_all_tickers_parallel(
        active_candidates, progress_callback=None, use_demo=use_demo, kite=kite, nifty_bullish=nifty_bullish
    )

...
def generate_synthetic_bearish_setup(ticker, pdc):
    ...
    for idx in range(40):
        if idx < 10:
            ...
            vol = np.random.randint(5000, 12000)
        elif idx == 22:
            price = 981.5
...
def run_rejection_scanner(df, pdc, pullback_threshold=0.0015, yesterday_low=None, nifty_bearish=False):
    ...
    # Relax yesterday's low filter to require being below PD Close and Today's Open (intraday negative trend)
    df_session['below_pdc_and_open'] = (df_session['close'] < pdc) & (df_session['close'] < df_session['open'].iloc[0])
    ...
    df_session['candle_ok'] = (df_session['close'] < (df_session['high'] - 0.6 * candle_range)) & (candle_range > 0)
